...
function generateIndexConfig() {
  try {
    ...
    const indexConfig = {
      ...
      // Índices para PDFDocument - Optimizados para alto rendimiento
      pdfdocuments: [
        { key: { s: 1 }, options: { background: true } }, // Índice por estado (el más usado)
        { key: { t: 1 }, options: { background: true } }, // Índice por título
        { key: { f: 1 }, options: { background: true } }, // Índice por nombre de archivo
        { key: { creado: -1 }, options: { background: true } }, // Índice por fecha de creación
        { key: { tx: "text", t: "text" }, options: { background: true, weights: { t: 10, tx: 5 } } } // Índice de texto para búsquedas
      ]
    };
