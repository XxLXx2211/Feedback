...
async function processPDF(filePath, options = {}) {
  try {
    ...
    // Obtener tamaño del archivo para optimizaciones
    ...
    const processingOptions = {
      ...
      extractTables: isLargeFile ? false : (options.extractTables !== false),
      extractForms: isLargeFile ? false : (options.extractForms !== false),
      extractImages: options.extractImages === true, // Desactivado por defecto para ahorrar recursos
      // Opciones de rendimiento
      priority: options.priority || (isLargeFile ? 'low' : 'normal'),
      useCache: options.useCache !== false,
      streamResponse: true // Usar streaming para mejor rendimiento
    };
    ...
    if (result.status === 'processed' && result.extraction && result.extraction.result_text) {
      ...
      // Extraer información relevante de forma optimizada
      const processedResult = {
        success: true,
        text: result.extraction.result_text || '',
        metadata: result.extraction.metadata || {},
        // Optimizar memoria para archivos grandes
        tables: isLargeFile ? [] : (result.extraction.tables || []),
        forms: isLargeFile ? [] : (result.extraction.forms || []),
        // No incluir imágenes a menos que se soliciten explícitamente
        images: options.extractImages ? (result.extraction.images || []) : [],
        pages: result.extraction.pages || 0,
        processingTime: result.processingTime || 0,
        // Incluir texto sin procesar solo si se solicita explícitamente
        rawText: options.includeRawText ? (result.extraction.raw_text || '') : '',
      ...
      // Solo si no es un archivo grande o si se solicita explícitamente
      if (!isLargeFile || options.enhanceText) {
