  ...
  useEffect(() => {
    const loadFeedback = async () => {
      try {
        ...
        // Ordenar las preguntas dentro de cada categoría: primero Si/No, luego Escala, finalmente Texto
        Object.keys(questionsByCategory).forEach(categoryId => {
          questionsByCategory[categoryId].questions.sort((a, b) => {
            // Definir el orden de prioridad
            const getTypeOrder = (type) => {
              if (type === 'si_no') return 1; // Si/No primero
              if (type === 'escala') return 2; // Escala segundo
              if (type === 'texto') return 3; // Texto tercero
              return 4; // Cualquier otro tipo al final
            };

            return getTypeOrder(a.tipo_respuesta) - getTypeOrder(b.tipo_respuesta);
          });
        });
        ...
        <div className="page-header">
          <div className="page-header-content text-center">
            ...
            <p className="page-description">
              Su opinión es importante para nosotros. Por favor, responde las siguientes preguntas. Gracias!
            </p>
          </div>
        </div>
